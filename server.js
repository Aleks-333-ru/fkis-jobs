...
// hh.ru понимает язык запросов с OR/NOT/кавычками.
// Положительная часть — профильные слова; NOT-часть сразу отсекает массовый
// «околоспортивный» шум (продажи, администраторы, торговля, вождение,
// активный отдых и т.п.), чтобы страницы hh не забивались мусором.
// isRelevant() ниже — вторая линия защиты.
const HH_POSITIVE = '(тренер OR инструктор OR фитнес OR "учитель физической культуры" OR "учитель физкультуры" OR "физическая культура" OR "спортивный")';
const HH_EXCLUDE = '(менеджер OR продавец OR продажам OR администратор OR ресепшен OR кассир OR консультант OR бизнес OR вождение OR маркетинг OR маркетолог OR бухгалтер OR финансовый OR закупки OR инженер OR эксплуатация OR визуализатор OR SMM OR официант OR бариста OR повар OR уборщик OR охранник OR грузчик OR кинолог OR пейнтбол OR картинг OR батут OR верёвочный OR сплав OR рафтинг)';
const HH_QUERY = `${HH_POSITIVE} NOT ${HH_EXCLUDE}`;
...
function isRelevant(title = '') {
  ...
  if (/трен[её]р|фитнес|физкультур|спорт|плавани|лыжн|гимнаст|атлет|единоборств|тренаж[её]р|оздоровит|(^|\s)гто(\s|$)|(^|\s)лфк(\s|$)|бассейн|физическ\S*\s+культ|групповы|пилатес|йог[аеиу]|растяжк|стретчинг|аэроб|кроссфит|зумб|хореограф|секци/.test(t)) return true;
